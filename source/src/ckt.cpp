...
namespace ckt_n {
    ...
    void ckt_t::_compare_keys(node_map_t& nm, const nodelist_t& i1, const nodelist_t& i2)
    {
        assert(i1.size() == i2.size());

        std::string name("__final_cmp_key__");
        std::string func("or");
        node_t* g_out = node_t::create_gate(name, func);
        add_gate(g_out);
    ...
    */
    ckt_t::ckt_t(nodepair_list_t& pair_map, nodepair_list_t& pair_map1)
    {   
        std::vector<node_t*> vec;
        assert(pair_map.size() == pair_map1.size());
        for(unsigned i=0; i != pair_map.size(); i++) {
            node_t* nA = pair_map[i].first;
            if(node_t::INPUT == nA->type && (!nA->is_keyinput()))
            {   
                if (vec.size() == 0){
                    vec.push_back(nA);
                    inputs.push_back(nA);
                ...
                else if(std::find(vec.begin(), vec.end(), nA) == vec.end()){
                    vec.push_back(nA);
                    inputs.push_back(nA);
                    ckt_inputs.push_back(nA);
                }
            }
            else if(node_t::INPUT == nA->type && nA->is_keyinput()){
                inputs.push_back(nA);
                key_inputs.push_back(nA);
            }
            else{
            ...
            node_t* nB = pair_map[i].second;
            if(node_t::INPUT == nB->type && (!nB->is_keyinput()))
            {   
                if (vec.size() == 0){
                    vec.push_back(nB);
                    inputs.push_back(nB);
                    ckt_inputs.push_back(nB);
                }
                else if(std::find(vec.begin(), vec.end(), nB) == vec.end()){
                    vec.push_back(nB);
                    inputs.push_back(nB);
                    ckt_inputs.push_back(nB);
                }
            }
            else if(node_t::INPUT == nB->type && nB->is_keyinput()){
                inputs.push_back(nB);
                key_inputs.push_back(nB);
            }
            else{
                assert(node_t::GATE == nB->type);
                gates.push_back(nB);
            }
            nodes.push_back(nB);

            node_t* nC = pair_map1[i].first;
            if(node_t::INPUT == nC->type && (!nC->is_keyinput()))
            {   
                if (vec.size() == 0){
                    vec.push_back(nC);
                    inputs.push_back(nC);
                    ckt_inputs.push_back(nC);
                }
                else if(std::find(vec.begin(), vec.end(), nC) == vec.end()){
                    vec.push_back(nC);
                    inputs.push_back(nC);
                    ckt_inputs.push_back(nC);
                }
            }
            else if(node_t::INPUT == nC->type && nC->is_keyinput()){
                inputs.push_back(nC);
                key_inputs.push_back(nC);
            }
            else{
                assert(node_t::GATE == nC->type);
                gates.push_back(nC);
            }
            nodes.push_back(nC);

            node_t* nD = pair_map1[i].second;
            if(node_t::INPUT == nD->type && (!nD->is_keyinput()))
            {   
                if (vec.size() == 0){
                    vec.push_back(nD);
                    inputs.push_back(nD);
                    ckt_inputs.push_back(nD);
                }
                else if(std::find(vec.begin(), vec.end(), nD) == vec.end()){
                    vec.push_back(nD);
                    inputs.push_back(nD);
                    ckt_inputs.push_back(nD);
                }
            }
            else if(node_t::INPUT == nD->type && nD->is_keyinput()){
                inputs.push_back(nD);
                key_inputs.push_back(nD);
            }
            else{
                assert(node_t::GATE == nD->type);
                gates.push_back(nD);
            }
            nodes.push_back(nD);
        }
