    ...
    , cube_count(0)
{
    ...
    for(unsigned i=0; i != ckt.num_key_inputs(); i++) {
        ...
        keyinput_literals_C.push_back(lmap[dbl.pair_map1[idx].first->get_index()]);
        keyinput_literals_D.push_back(lmap[dbl.pair_map1[idx].second->get_index()]);
    }
    ...
    for(unsigned i=0; i != ckt.num_outputs(); i++) {
        ...
        node_t* outC = dbl.pair_map1[idx].first;
        node_t* outD = dbl.pair_map1[idx].second;
        Lit lA = lmap[outA->get_index()];
        Lit lB = lmap[outB->get_index()];
        Lit lC = lmap[outC->get_index()];
        Lit lD = lmap[outD->get_index()];
        output_literals_A.push_back(lA);
        output_literals_B.push_back(lB);
        output_literals_C.push_back(lC);
        output_literals_D.push_back(lD);
    }

    S.freeze(keyinput_literals_A);
    S.freeze(keyinput_literals_B);
    S.freeze(keyinput_literals_C);
    S.freeze(keyinput_literals_D);

    S.freeze(cktinput_literals);

    S.freeze(output_literals_A);
    S.freeze(output_literals_B);
    S.freeze(output_literals_C);
    S.freeze(output_literals_D);
...
void solver_t::addKnownKeys(std::vector<std::pair<int, int> >& values)
{
    for(unsigned i=0; i != values.size(); i++) {
        ...
        Lit lC = keyinput_literals_C[values[i].first];
        Lit lD = keyinput_literals_D[values[i].first];
...
)
{
    ...
    for(unsigned i=0; i != ckt.num_outputs(); i++) {
        ...
        int idxC = dbl.pair_map1[idx].first->get_index();
        int idxD = dbl.pair_map1[idx].second->get_index();

        Var vA = var(lmap[idxA]);
        Var vB = var(lmap[idxB]);

        Var vC = var(lmap[idxC]);
        Var vD = var(lmap[idxD]);

        assert(vA < values.size() && vA >= 0);
        assert(vB < values.size() && vB >= 0);
        assert(vC < values.size() && vC >= 0);
        assert(vD < values.size() && vD >= 0);

        if(output_values[i] == true) {
            values[vA] = values[vB] = values[vC] = values[vD] = sat_n::l_True;
        } else {
            values[vA] = values[vB] = values[vC] = values[vD] = sat_n::l_False;
        }
    }
}
...
void solver_t::blockKey(rmap_t& keysFound)
{
    ...
    vec_lit_t bc_C(keyinput_literals_A.size());
    vec_lit_t bc_D(keyinput_literals_A.size());
    ...
    for(unsigned i=0; i != keyinput_literals_A.size(); i++) {
        ...
        bc_C[i] = ki ? ~keyinput_literals_C[i] : keyinput_literals_C[i];
        bc_D[i] = ki ? ~keyinput_literals_D[i] : keyinput_literals_D[i];
    }
    S.addClause(bc_A);
    S.addClause(bc_B);
    S.addClause(bc_C);
...
{
    using namespace sat_n;
    vec_lit_t c1, c2, c3, c4;

    assert(keyinput_literals_A.size() == keyinput_literals_B.size());
    assert(keyinput_literals_C.size() == keyinput_literals_D.size());
    assert(keyinput_literals_A.size() == keyinput_literals_C.size());

    c1.growTo(keyinput_literals_A.size()+1);
    c2.growTo(keyinput_literals_B.size()+1);
    c3.growTo(keyinput_literals_C.size()+1);
    c4.growTo(keyinput_literals_D.size()+1);

    for(unsigned i=0; i != keyinput_literals_A.size(); i++) {
        c1[i+1] = ~keyinput_literals_A[i];
        c2[i+1] = ~keyinput_literals_B[i];
        c3[i+1] = ~keyinput_literals_C[i];
        c4[i+1] = ~keyinput_literals_D[i];
    }
    c1[0] = l_out;
    c2[0] = l_out;
    c3[0] = l_out;
    c4[0] = l_out;
    if(S.solve(c1) == false && S.solve(c2) == false && S.solve(c3) == false && S.solve(c4) == false) {
        return true;
    } else {
        return true;
    }
